...
function read (name, ver, forceBypass, cb) {
  ...
  var root = cachedPackageRoot(npm, {name: name, version: ver})
  function c (er, data) {
  ...
  if (forceBypass && npm.config.get('force')) {
    ...
    return addNamed(npm, name, ver, null, c)
  }
  ...
  readJson(path.join(root, 'package', 'package.json'), function (er, data) {
    ...
    if (er) return addNamed(npm, name, ver, null, c)
    else c(er, data)
  })
}
    ...
    switch (p.type) {
      ...
      case 'remote':
        ...
        mapToRegistry(p.raw, npm.config, function (err, uri, auth) {
          ...
          addRemoteTarball(npm, p.spec, { name: p.name }, null, auth, cb)
        })
        break
      case 'git':
      case 'hosted':
        addRemoteGit(npm, p.rawSpec, cb)
        break
      default:
        if (p.name) return addNamed(npm, p.name, p.spec, null, cb)
...
function unpack (pkg, ver, unpackTarget, dMode, fMode, uid, gid, cb) {
  ...
  read(pkg, ver, false, function (er) {
    ...
    npm.commands.unbuild([unpackTarget], true, function (er) {
      if (er) return cb(er)
      tar.unpack(
        path.join(cachedPackageRoot(npm, { name: pkg, version: ver }), 'package.tgz'),
        unpackTarget,
        dMode, fMode,
        uid, gid,
        cb
...
function afterAdd (cb) {
  return function (er, data) {
    ...
    // Ignore if it fails.
    var pj = path.join(cachedPackageRoot(npm, data), 'package', 'package.json')
