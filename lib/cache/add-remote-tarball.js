...
function addRemoteTarball (npm, u, pkgData, shasum, auth, cb_) {
  ...
  cb_ = inflight(u, cb_)
  if (!cb_) return npm.log.verbose('addRemoteTarball', u, 'already in flight; waiting')
  npm.log.verbose('addRemoteTarball', u, 'not in flight; adding')
  ...
  function next (er, resp, shasum) {
    if (er) return cb(er)
    addLocalTarball(npm, tmp, pkgData, shasum, cleanup)
  }
  ...
  npm.log.verbose('addRemoteTarball', [u, shasum])
  mkdir(path.dirname(tmp), function (er) {
    if (er) return cb(er)
    addRemoteTarball_(npm, u, tmp, shasum, auth, next)
  })
}

function addRemoteTarball_ (npm, u, tmp, shasum, auth, cb) {
  ...
  operation.attempt(function (currentAttempt) {
    ...
    fetchAndShaCheck(npm, u, tmp, shasum, auth, function (er, response, shasum) {
      // Only retry on 408, 5xx or no `response`.
      var sc = response && response.statusCode
      var statusRetry = !sc || (sc === 408 || sc >= 500)
      if (er && statusRetry && operation.retry(er)) {
        npm.log.warn('retry', 'will retry, error on last attempt: ' + er)
        return
      }
    ...
    tarball.on('finish', function () {
      if (!shasum) {
        ...
        return sha.get(tmp, function (er, shasum) {
          npm.log.silly('fetchAndShaCheck', 'shasum', shasum)
          cb(er, response, shasum)
        })
      }
