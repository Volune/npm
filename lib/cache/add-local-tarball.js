...
function addLocalTarball (npm, p, pkgData, shasum, cb) {
  ...
  if (!shasum) {
    return sha.get(p, function (er, shasum) {
      if (er) return cb(er)
      npm.log.silly('addLocalTarball', 'shasum (computed)', shasum)
      addLocalTarball(npm, p, pkgData, shasum, cb)
    })
  }
  ...
  if (pathIsInside(p, npm.cache)) {
    ...
    npm.log.verbose('addLocalTarball', 'adding from inside cache', p)
    return addPlacedTarball(npm, p, pkgData, shasum, cb)
  }

  addTmpTarball(npm, p, pkgData, shasum, function (er, data) {
    if (data) {
      data._resolved = p
...
function addPlacedTarball (npm, p, pkgData, shasum, cb) {
  assert(pkgData, 'should have package data by now')
  assert(typeof cb === 'function', 'cb function required')

  getCacheStat(npm, function (er, cs) {
    if (er) return cb(er)
    return addPlacedTarball_(npm, p, pkgData, cs.uid, cs.gid, shasum, cb)
  })
}

function addPlacedTarball_ (npm, p, pkgData, uid, gid, resolvedSum, cb) {
  var folder = path.join(cachedPackageRoot(npm, pkgData), 'package')

  // First, make sure we have the shasum, if we don't already.
  if (!resolvedSum) {
    sha.get(p, function (er, shasum) {
      if (er) return cb(er)
      addPlacedTarball_(npm, p, pkgData, uid, gid, shasum, cb)
    })
    return
  }
...
function addTmpTarball (npm, tgz, pkgData, shasum, cb) {
  ...
  if (!cb) return npm.log.verbose('addTmpTarball', tgz, 'already in flight; not adding')
  npm.log.verbose('addTmpTarball', tgz, 'not in flight; adding')
  ...
  if (pkgData && pkgData.name && pkgData.version) {
    ...
    return addTmpTarball_(npm, tgz, pkgData, shasum, cb)
  }
  ...
  getCacheStat(npm, function (er, cs) {
    ...
    npm.log.verbose('addTmpTarball', 'validating metadata from', tgz)
    tar.unpack(tgz, target, null, null, cs.uid, cs.gid, function (unpackEr, data) {
      ...
      rimraf(target, function () {
        ...
        addTmpTarball_(npm, tgz, data, shasum, cb)
      })
    })
  })
}

function addTmpTarball_ (npm, tgz, data, shasum, cb) {
  assert(typeof cb === 'function', 'must have callback function')
  cb = once(cb)
  ...
  var root = cachedPackageRoot(npm, data)
