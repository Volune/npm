 ...
 */
function updateIndex (npm, staleness, cb) {
  ...
  mapToRegistry('-/all', npm.config, function (er, uri, auth) {
    ...
    npm.log.info('updateIndex', cachePath)

    getCacheStat(npm, function (er, st) {
      if (er) return cb(er)
      ...
      mkdir(cacheBase, function (er, made) {
        ...
        fs.readFile(cachePath, function (er, data) {
          if (er) {
            npm.log.warn('', 'Building the local index for the first time, please be patient')
            return updateIndex_(npm, uri, params, {}, cachePath, cb)
          }
          ...
          chownr(made || cachePath, st.uid, st.gid, function (er) {
            ...
            if (t === 0) {
              ...
              npm.log.verbose('updateIndex', 'Cached search data present with timestamp', t)
              uri += '/since?stale=update_after&startkey=' + t
            }
            updateIndex_(npm, uri, params, data, cachePath, cb)
          })
        })
      })
    })
...
function updateIndex_ (npm, all, params, data, cachePath, cb) {
  npm.log.silly('update-index', 'fetching', all)
