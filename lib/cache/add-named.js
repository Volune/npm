...
function getOnceFromRegistry (npm, name, from, next, done) {
  ...
  mapToRegistry(name, npm.config, function (er, uri, auth) {
    ...
    if (!next) return npm.log.verbose(from, key, 'already in flight; waiting')
    else npm.log.verbose(from, key, 'not in flight; fetching')

    npm.registry.get(uri, { auth: auth }, pulseTillDone('fetchRegistry', fixName))
  })
}

function addNamed (npm, name, version, data, cb_) {
  assert(typeof name === 'string', 'must have module name')
  assert(typeof cb_ === 'function', 'must have callback')

  var key = name + '@' + version
  npm.log.silly('addNamed', key)
  ...
  if (semver.valid(version, true)) {
    npm.log.verbose('addNamed', JSON.stringify(version), 'is a plain semver version for', name)
    addNameVersion(npm, name, version, data, cb)
  } else if (semver.validRange(version, true)) {
    npm.log.verbose('addNamed', JSON.stringify(version), 'is a valid semver range for', name)
    addNameRange(npm, name, version, data, cb)
  } else {
    npm.log.verbose('addNamed', JSON.stringify(version), 'is being treated as a dist-tag for', name)
    addNameTag(npm, name, version, data, cb)
  }
}

function addNameTag (npm, name, tag, data, cb) {
  npm.log.info('addNameTag', [name, tag])
  var explicit = true
  if (!tag) {
    explicit = false
  ...
  getOnceFromRegistry(npm, name, 'addNameTag', next, cb)

  function next (er, data, json, resp) {
    if (!er) er = errorResponse(name, resp)
    if (er) return cb(er)

    npm.log.silly('addNameTag', 'next cb for', name, 'with tag', tag)

    engineFilter(npm, data)
    if (data['dist-tags'] && data['dist-tags'][tag] &&
        data.versions[data['dist-tags'][tag]]) {
      var ver = data['dist-tags'][tag]
      return addNamed(npm, name, ver, data.versions[ver], cb)
    }
    if (!explicit && Object.keys(data.versions).length) {
      return addNamed(npm, name, '*', data, cb)
    }
...
function engineFilter (npm, data) {
...
function addNameVersion (npm, name, v, data, cb) {
  ...
  getOnceFromRegistry(npm, name, 'addNameVersion', setData, cb)
  ...
  function next () {
    ...
    var pkgroot = cachedPackageRoot(npm, { name: name, version: ver })
    ...
    function fetchit () {
      mapToRegistry(name, npm.config, function (er, _, auth, ruri) {
        ...
        addRemoteTarball(npm, tb, data, dist.shasum, auth, cb)
      })
    }
  }
}

function addNameRange (npm, name, range, data, cb) {
  ...
  npm.log.silly('addNameRange', { name: name, range: range, hasData: !!data })

  if (data) return next()

  getOnceFromRegistry(npm, name, 'addNameRange', setData, cb)
  ...
  function next () {
    ...
    npm.log.silly('addNameRange', 'versions'
    ...
    if (tagged &&
      ...
      return addNamed(npm, name, tagged, data.versions[tagged], cb)
    }
    ...
    if (!ms) {
      if (range === '*' && versions.length) {
        return addNameTag(npm, name, 'latest', data, cb)
      } else {
    ...
    addNamed(npm, name, ms, data.versions[ms], cb)
  }
}
