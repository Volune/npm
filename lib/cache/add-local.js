...
function addLocal (npm, p, pkgData, cb_) {
  ...
  function cb (er, data) {
    if (er) {
      npm.log.error('addLocal', 'Could not install %s', p.spec)
      return cb_(er)
    }
  ...
  if (p.type === 'directory') {
    addLocalDirectory(npm, p.spec, pkgData, null, cb)
  } else {
    addLocalTarball(npm, p.spec, pkgData, null, cb)
  }
}
...
function addLocalDirectory (npm, p, pkgData, shasum, cb) {
  ...
  readJson(path.join(p, 'package.json'), false, function (er, data) {
    ...
    if (!wrapped) return npm.log.verbose('addLocalDirectory', tgz, 'already in flight; waiting')
    npm.log.verbose('addLocalDirectory', tgz, 'not in flight; packing')
      ...
      mkdir(path.dirname(pj), function (er, made) {
        ...
        function thenPack () {
          tar.pack(tgz, p, data, function (er) {
            if (er) {
              npm.log.error('addLocalDirectory', 'Could not pack', p, 'to', tgz)
              return wrapped(er)
            }
    ...
    function next (er) {
      ...
      if (shasum) {
        return addLocalTarball(npm, tgz, data, shasum, cb)
      } else {
